...
- **절대 상대방을 주어로 사용하지 마세요.** "상대방이 ~했으면 좋겠어요", "~이 ~하길 바랐어요" 같은 표현은 절대 사용 금지입니다.
- **욕구는 "내가 ~받고 싶어요", "내가 ~하고 싶어요" 형태로만 표현하세요.**
...
- "상대방이 친절하게 대했으면 좋겠어요" ❌ (절대 안 됨 - 상대방 중심)
- "내가 친절한 대우를 받고 싶어요" ✅ (올바름 - 내가 받고 싶은 것)
- "아저씨가 평소 목소리로 말했으면 좋겠어요" ❌ (절대 안 됨 - 상대방 중심)
- "내가 존중받고 싶어요" ✅ (올바름 - 내가 받고 싶은 것)
- "제 주변 환경이 안전했으면 좋겠어요" ✅
- "저는 이해받고 싶어요" ✅
...
위 정보를 바탕으로 자연스럽고 읽기 쉬운 한국어 메시지를 작성해주세요.`
    ...
    if (generatedMessage && generatedMessage.trim().length > 0) {
      ...
      // 2. 욕구 표현 오류 수정 (포괄적 및 강력하게)
      // 상대방 중심 표현을 "나" 중심으로 변환
      // "후임이 ~하길 바랐어요" → "내가 후임으로부터 ~받고 싶었어요" 형태로 변환
      ...
      // "상대방이 ~했으면 좋겠어요" → "내가 ~받고 싶었어요"
      nvcMessage = nvcMessage.replace(/상대방이 ([^가-힣]+)했으면 좋겠어요/g, '내가 $1받고 싶었어요');
      nvcMessage = nvcMessage.replace(/아저씨가 ([^가-힣]+)했으면 좋겠어요/g, '내가 $1받고 싶었어요');
      nvcMessage = nvcMessage.replace(/기사가 ([^가-힣]+)했으면 좋겠어요/g, '내가 $1받고 싶었어요');
      // "내가 상대방이 ~하길" 같은 혼합 표현 제거
      nvcMessage = nvcMessage.replace(/내가 상대방이 ([^가-힣]+)하길 바랐어요/g, '내가 $1받고 싶었어요');
      nvcMessage = nvcMessage.replace(/저는 상대방이 ([^가-힣]+)하길 바랐어요/g, '저는 $1받고 싶었어요');
...
4. **절대 상대방을 주어로 사용하지 마세요.** "후임이 ~하길 바랐어요", "상대방이 ~했으면 좋겠어요" 같은 표현은 절대 사용 금지입니다.
5. **욕구는 "내가 ~받고 싶었어요", "내가 ~하고 싶었어요", "저는 ~받고 싶었어요" 형태로만 표현하세요.**
6. **절대 "상대방이 ~했으면 좋겠어요", "~이 ~하길 바랐어요" 같은 상대방 중심 표현을 사용하지 마세요.**

**올바른 욕구 표현 (주체가 "나", 내가 받고 싶은 것):**
- "내가 존중받고 싶었어요" ✅
- "내가 이해받고 싶었어요" ✅
- "내가 배려받고 싶었어요" ✅
- "내가 친절한 대우를 받고 싶었어요" ✅
...
- "상대방이 예의를 지켜주길 바랐어요" ❌ (상대방을 주어로 사용)
- "상대방이 친절하게 대했으면 좋겠어요" ❌ (상대방 중심)
- "아저씨가 평소 목소리로 말했으면 좋겠어요" ❌ (상대방 중심)
- "팀원들이 협력하길 바랐어요" ❌ (상대방을 주어로 사용)
- "내가 상대방이 ~하길 바랐어요" ❌ (혼합 표현, 절대 안 됨)

**핵심 원칙:**
- 욕구는 "내가 받고 싶은 것", "내가 느끼고 싶은 것"만 표현하세요.
- 상대방의 행동이나 태도를 요구하는 표현은 절대 사용하지 마세요.
- "~받고 싶었어요", "~하고 싶었어요", "~느끼고 싶었어요" 형태만 사용하세요.

상황과 감정에 가장 적합한 욕구만 선택하세요. **반드시 주체가 "나(저)"이고, 내가 받고 싶은 것만 표현하세요.**`
        },
        {
          role: "user",
    ...
    // 상대방 중심 표현 필터링 (더 강력하게)
    filteredNeeds = filteredNeeds.filter(need => {
      const lowerNeed = need.toLowerCase();
      // 상대방을 주어로 사용한 표현 제외
      if (lowerNeed.includes('이 ') && (lowerNeed.includes('하길 바랐') || lowerNeed.includes('했으면 좋겠'))) {
        return false;
      }
      if (lowerNeed.includes('가 ') && (lowerNeed.includes('하길 바랐') || lowerNeed.includes('했으면 좋겠'))) {
        return false;
      }
      if (lowerNeed.includes('상대방이') || lowerNeed.includes('상대방이')) {
        return false;
      }
      if (lowerNeed.includes('아저씨가') || lowerNeed.includes('기사가')) {
        return false;
      }
      // "내가 상대방이 ~하길" 같은 혼합 표현 제외
      if (lowerNeed.includes('내가') && (lowerNeed.includes('상대방이') || lowerNeed.includes('하길 바랐'))) {
        return false;
      }
      return true;
    });
